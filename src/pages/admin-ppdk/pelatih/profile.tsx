	...
	return (
		<Layout>
			<Tabs defaultValue="profil">
				<TabsList>
					<TabsTrigger value="profil">Profil Pelatih</TabsTrigger>
				...
				<TabsContent value="profil">
					<div className="grid grid-cols-3 gap-4 mb-4">
						...
						<div className="col-span-3 text-right">
							<Button
								onClick={() =>
									navigate(`/admin-ppdk/pelatih-update/${id}`)
								}
								variant="outline"
							>
								<Pencil size={16} />
				...
				<TabsContent value="penilaian">
					<Table>
						...
						<TableBody>
							{profile?.assessment?.map((assess, index) => (
								<TableRow key={assess.id}>
									<TableCell>{index + 1}</TableCell>
									<TableCell>
										{assess.kategori_oku.kategori}
									</TableCell>
									<TableCell className="text-center">
										{assess?.skor || "-"}
									</TableCell>
									<TableCell className="text-right">
										{dayjs(assess?.created_at).format(
											"DD/MM/YYYY HH:mm"
										)}
									</TableCell>
									<TableCell className="text-right">
